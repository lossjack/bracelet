...
void IIC2_WriteByte(unsigned char WData)
{
	...
	for(int i=0;i<8;i++)
	{
		SCL_1(0);//SCL低电平期间 发送者可以改变SDA发送一个位
		delay_us(2);
		if(WData & (0x80>>i))
			SDA_W1(1);//表示是数字1
		else
		...
		SCL_1(1);//SCL高电平期间 是给接受者读取SDA的时间
		delay_us(5);
	}
	
	SCL_1(0);//最后要保证SCL是低电平结束本轮
}
...
unsigned char IIC2_ReadByte(void)
{
	...
	for(int i=0;i<8;i++)
	{
		SCL_1(0);//从机在SCL低电平期间改变SDA发送
		delay_us(5);//从机发送数据位的期间
		
		
		SCL_1(1);//主机在高电平判断SDA电平读取
		delay_us(2);
		if(SDA_R1)
			RData |= (0x80>>i);//数字1
		delay_us(3);
