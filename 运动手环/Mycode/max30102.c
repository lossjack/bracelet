...
u8 max30102_Bus_Write(u8 Register_Address, u8 Word_Data)
{
	...
	IIC2_WriteByte(max30102_WR_address | I2C_WR);	/* 此处是写指令 */
...
u8 max30102_Bus_Read(u8 Register_Address)
{
	...
	IIC2_WriteByte(max30102_WR_address | I2C_RD);	/* 此处是读指令 */
	...
	/* 第9步：读取数据 */
	{
		data = IIC2_ReadByte();	/* 读1个字节 */
 
		IIC2_WriteAck(1);	/* 最后1个字节读完后，CPU产生NACK信号(驱动SDA = 1) */
	}
	/* 发送I2C总线停止信号 */
	IIC2_Stop();
...
{		
	...
	IIC2_WriteByte(max30102_WR_address);	    //发送写命令
	IIC2_ReadAck();
	
	IIC2_WriteByte(WriteAddr);	    //发送写命令
	IIC2_ReadAck();
...
{		
	...
	IIC2_Start(); 
	IIC2_WriteByte(max30102_WR_address|0X01);//进入接收模式			   
	IIC2_ReadAck();
